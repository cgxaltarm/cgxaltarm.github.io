...
import { filterModelsListForManagedKey } from "@/dailyflo/policy";
...
export async function GET(request) {
  try {
    ...
    if (token.startsWith("df_live_")) {
      const auth = await verifyManagedApiKey(token);
      if (!auth.valid) {
        return Response.json({ error: { message: "Invalid API key", type: "invalid_request_error" } }, { status: 401 });
      }
      data = await filterModelsListForManagedKey(auth.key, data);
    }
